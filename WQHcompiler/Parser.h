...
public:
    ...
    struct Symbol_item
    {
        long long token =0;      // 标记，值应该是Token_type类型的，除了关键字之外，一定是Id Num 或者运算符。
        long long hash=0;       // 根据名称计算出的一个哈希值，加速查找，不需要每次都去遍历名字比较。
        char* name=0;     // 名称，计算出哈希值之后就不需要用它了，指向源文件某位置的char*指针。
        long long IdClass=0;      // 标识符的属性，Id类型的token才需要，值为Identifier_type中枚举。
    ...
    struct Symbol_item* idmain=0;         // main函数的符号表记录
     struct Symbol_item* current_id=0;         // 当前标识符的符号表记录 也就是这东西是在不断维护的
    long long basetype=0;                       // 变量、函数和类型定义的基本类型，是指针类型时使用
    long long expr_type=0;                      // 表达式类型
    long long index_of_bp=0;                    // 函数调用时第一个参数相对bp的位置，即为函数的参数数量+1
    ...
    struct Func_call_item* func_list=0;   // 待填充的函数调用列表 注意这里只有声明定义分离的才会在表里
	// union or struct domain ，用来处理struct和union的域
    struct us_domain
    {
        ...
        struct us_domain* next=0; // first domain for type or next domain for domain, 指示是否已定义struct或者union 不是0，表示已经定义
        //这个链表链起来的时候其实就是存储了一个struct的成员
    };

    // 结构体和联合体信息的列表，每个元素都是一个链表的表头，存储struct和union的信息，链表中余下的节点按顺序存储它的域 虽然是列表，但是其实只是数组
    ...
    long long cur_struct_type=0;        // 当前struct类型，每出现一个新的struct就++一次，这样也保证减掉STRUCT后就是struct_symbols_list的下标
...
public:
    Parser(VM& testVM,long long input_poolsize = 512 * 1024);//512kb的符号表,源代码
    ~Parser();
    void init_symbols();
